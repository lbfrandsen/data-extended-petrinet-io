export default class SqlParserService {
  ...
  executeAttachedActionForTransition(transition, binding = {}) {
    ...
    if (!this.databaseService.getDbName()) {
      return { ok: false, skipped: false, error: 'No database loaded. Upload a .db/.sqlite file first.' };
    }
    ...
    for (const actionId of actionIds) {
      ...
      if (!entry) {
        return { ok: false, skipped: false, error: `Action ${normalizedActionId} not found.` };
      }

      const validationError = this._validateActionEntry(entry, normalizedActionId);
      if (validationError) {
        return { ok: false, skipped: false, error: validationError };
      }

      preparedActions.push({
        id: normalizedActionId,
        type: String(entry?.type || '').toUpperCase() === 'DELETE' ? 'DELETE' : 'INSERT',
        sql: this._buildActionSqlWithBindings(String(entry.sql || '').trim(), binding)
      });
    }

    const orderedActions = this._sortActionBatch(preparedActions);
    const sqlBatch = orderedActions.map((action) => action.sql);
    const execution = this.databaseService.runStatementsAtomically(sqlBatch, `transition_${transition?.id || 'fire'}`);
  ...
  executeActionById(actionId, binding = {}) {
    const normalizedActionId = String(actionId || '').trim();
    ...
    const sqlToExecute = this._buildActionSqlWithBindings(entry.sql, binding);
    const result = this.databaseService.queryWithStatus(sqlToExecute);
    if (!result.ok) {
  ...
  _sortActionBatch(actions = []) {
    return [...actions].sort((a, b) => {
      const rank = (action) => (action?.type === 'DELETE' ? 0 : 1);
      return rank(a) - rank(b);
    });
  }

  _buildActionSqlWithBindings(sql, binding) {
    if (!sql || typeof sql !== 'string' || !binding || typeof binding !== 'object') {
      return sql;
    }

    return Object.entries(binding).reduce((currentSql, [key, value]) => {
      const replacement = this._formatActionBindingValue(value);
      const pattern = new RegExp(`\\b${this._escapeRegExp(String(key))}\\b`, 'g');
      return currentSql.replace(pattern, (match, offset) => {
        const start = Number(offset);
        const end = start + String(match).length;

        if (!this._shouldReplaceActionBindingOccurrence(currentSql, start, end, key)) {
          return match;
        }
  ...
  _shouldReplaceActionBindingOccurrence(sql, start, end, key) {
    if (sql[start - 1] === '.' || sql[end] === '.') {
      return false;
    }

    if (this._isSameNameAssignmentLeftOperand(sql, start, end, key)) {
      return false;
    }
  ...
  _isSameNameAssignmentLeftOperand(sql, start, end, key) {
    const afterLeft = String(sql).slice(end);
    const escapedKey = this._escapeRegExp(String(key));
    const sameNameAssignment = new RegExp(`^\\s*=\\s*${escapedKey}\\b`);

    if (!sameNameAssignment.test(afterLeft)) {
      return false;
    }

    const rightStart = end + (afterLeft.match(/^\s*=\s*/)?.[0]?.length || 0);
    const rightEnd = rightStart + String(key).length;

    return sql[rightStart - 1] !== '.' && sql[rightEnd] !== '.';
  }

  _formatActionBindingValue(value) {
