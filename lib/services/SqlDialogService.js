...
import { showDatabaseExplorerDialog } from './DatabaseExplorerDialogService.js';

let activeSqlDialogService = null;

export function getQueryById(queryId) {
  return activeSqlDialogService?.getQueryById?.(queryId);
}
...
export default class SqlDialogService {
  ...
  constructor(eventBus, databaseService) {
    ...
    this.eventBus.on('shape.removed', ({ element } = {}) => {
      if (element?.type === 'petri:transition') {
        this.detachAllBindingsForTransition(element);
      }
    });
    ...
    this.container.addEventListener('mouseleave', () => this._collapseOnMouseLeave());
  ...
  // Collapses when leaving hover only in "no database" mode. With a DB connected, panel stays pinned until explicit collapse.
  _collapseOnMouseLeave() {
    if (!this.isExpanded) {
      return;
    }
  ...
  detachAllBindingsForTransition(transitionOrId) {
    const detachedQuery = this.detachQueryFromTransition(transitionOrId);
    const detachedActions = this.detachActionFromTransition(transitionOrId);
    const changed = Boolean(detachedQuery || detachedActions);
  ...
  _createEntry(query = '') {
    return {
      id: `Q${this._getNextAvailableNumber(this.queries, 'Q')}`,
      text: String(query)
    };
  }
  ...
  _createActionEntry(type = 'INSERT', sql = '') {
    ...
    return {
      id: `A${this._getNextAvailableNumber(this.actionEntries, 'A')}`,
      type: normalizedType,
      sql: normalizedSql,
  ...
  _getNextAvailableNumber(entries = [], prefix) {
    ...
    if (prefix === 'Q') {
      this._nextQueryNumber = candidate + 1;
    } else if (prefix === 'A') {
      this._nextActionNumber = candidate + 1;
    }
  ...
  _buildTabbedHeader(parent) {
    ...
    previewButton.className = 'sql-dialog__preview-db';
    previewButton.textContent = 'PREVIEW';
    previewButton.title = 'Open database explorer';
    previewButton.addEventListener('click', async () => {
      await showDatabaseExplorerDialog(this.databaseService);
    });
  ...
  // Temporarily expands a structured action input for editing, then restores the compact row on Enter.
  _wireExpandableActionInput(input) {
    const collapse = () => {
      input.classList.remove('is-expanded');
      input.closest('.sql-dialog__action-row')?.classList.remove('is-editing-action-input');
    };

    input.addEventListener('focus', () => {
      input.closest('.sql-dialog__action-row')?.classList.add('is-editing-action-input');
  ...
  _buildActionsTab(parent) {
    ...
    if (!this.actionEntries.length) {
      ...
      this.actionEntries.forEach((entry) => {
        ...
        if (entry.type === 'DELETE') {
          ...
          this._wireExpandableActionInput(fromInput);
          ...
          this._wireExpandableActionInput(whereInput);
          ...
          this._wireExpandableActionInput(intoInput);
          ...
          this._wireExpandableActionInput(valuesInput);
    ...
    warning.textContent = 'When typing SQL statements in Actions, presser \'Enter\' to collapse input field.\nCAUTION: Actions modify your database irreversibly upon next export. Please be careful, and always have a backeup database.';
  ...
  // Builds the query output panel section.
  buildOutput(parent) {
    const divider = document.createElement('div');
