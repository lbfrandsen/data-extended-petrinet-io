...
export default class SqlDialogService {
  ...
  constructor(eventBus, databaseService) {
    ...
    this.eventBus.on('sqlDialog.open', ({ element } = {}) => {
      ...
      const selectedActionId = this.getBoundActionIdForTransition(element);
      showSqlQueriesList({
        ...
        onSelectAction: (actionId) => {
          if (selectedActionId && selectedActionId === actionId) {
            const detached = this.detachActionFromTransition(element);
            if (detached && element) {
              this.eventBus.fire('element.changed', { element });
              this.render();
            }
  ...
  // Expands the SQL panel on hover when it is currently collapsed. This keeps the first-open interaction lightweight without requiring a click.
  _expandFromHover() {
    if (this.isExpanded) {
      return;
    }
  ...
  // Collapses the SQL panel and returns it to hover-open mode. This is the explicit close action once the panel is pinned open.
  _collapsePanel() {
    this.isExpanded = false;
    this.render();
  }
  ...
  loadActionBindings(bindings = []) {
    ...
    bindings.forEach((binding) => {
      ...
      if (this.getActionEntryById(binding.actionId)) {
        this.actionBindings.set(binding.transitionId, binding.actionId);
      }
    });
  ...
  attachActionToTransition(transitionOrId, actionId) {
    ...
    this.actionBindings.set(transitionId, actionId);
    this._setActionBindingOnBusinessObject(transitionOrId, actionId);
    this._emitBindingsChanged();
    return true;
  }

  // Detaches any action bound to a transition. It also clears the business object property mirror.
  detachActionFromTransition(transitionOrId) {
    const transitionId = this._resolveTransitionId(transitionOrId);
    if (!transitionId) {
      return false;
    }

    const hadBinding = this.actionBindings.delete(transitionId);
  ...
  getBoundActionIdForTransition(transitionOrId) {
    ...
    return this.actionBindings.get(transitionId);
  }
  ...
  getBoundActionEntryForTransition(transitionOrId) {
    const actionId = this.getBoundActionIdForTransition(transitionOrId);
    if (!actionId) {
      return undefined;
    }
  ...
  getActionBindings() {
    return Array.from(this.actionBindings.entries()).map(([transitionId, actionId]) => ({
      transitionId,
      actionId
    }));
  }
  ...
  _setActionBindingOnBusinessObject(transitionOrId, actionId) {
    if (typeof transitionOrId === 'string') {
      return;
    }
    ...
    if (actionId) {
      businessObject.actionId = actionId;
      return;
    }
  ...
  _removeBindingsForAction(actionId) {
    ...
    for (const [transitionId, boundActionId] of this.actionBindings.entries()) {
  ...
  // Builds query-tab footer controls for adding and clearing queries. This keeps query list actions grouped in one section.
