...
function createButton(label, onClick, kind = 'secondary') {
  ...
  if (kind === 'primary') {
    ...
    button.style.color = '#fff';
    button.style.border = '1px solid #c62828';
  } else {
...
function formatRealInputValue(value) {
...
function createRealDomainInputs(pendingRules) {
  ...
  minInput.dataset.ruleField = 'realDomainMin';
  minInput.placeholder = 'Min';
  minInput.value = formatRealInputValue(pendingRules.realDomainMin);
  minInput.addEventListener('change', () => { pendingRules.realDomainMin = Number(minInput.value); });

  const maxInput = document.createElement('input');
  maxInput.type = 'text';
  maxInput.inputMode = 'decimal';
  maxInput.dataset.ruleField = 'realDomainMax';
  maxInput.placeholder = 'Max';
  maxInput.value = formatRealInputValue(pendingRules.realDomainMax);
...
export function showRulesDialog({ title = '', message = '', onClose = null }) {
  ...
  message = `Set rules for token value consumption and production when firing transitions.\n\n`;
  ...
  boolRow.textContent = 'Boolean: Fair Coin Toss';
...
export function showAlert({ title = 'Message', message = '', contentHtml = null }) {
...
export function showConfirmDialog({
  ...
  return new Promise(resolve => {
    ...
    const buttons = createButtonRow();
    const cancelButton = createButton(cancelLabel, () => closeDialog(false));
    const confirmButton = createButton(confirmLabel, () => closeDialog(true), confirmKind);
...
export function showResetModeDialog({
  ...
  return new Promise(resolve => {
    ...
    const setMasterButton = createButton('Set New Master Baseline', () => closeDialog('set-master-baseline'), 'danger');

    const rightButtons = document.createElement('div');
    rightButtons.style.display = 'flex';
    rightButtons.style.gap = '10px';
    if (canSetMasterBaseline) {
      rightButtons.appendChild(setMasterButton);
    }
...
export function showMultiPrompt({
  ...
  return new Promise(resolve => {
    ...
    buttons.appendChild(
      createButton(submitLabel, submit, 'primary')
    );
  ...
  return new Promise(resolve => {
    ...
    const renderConsumptionSection = (rowBinding) => {
      ...
      incomingArcs.forEach((connection, index) => {
        ...
        // Skip arcs with no variable (empty inscription) or whose variables are all already bound by the query result row
        const rawInscription = connection?.businessObject?.arcInscription || '';
        const arcVars = rawInscription.replace(/[<>]/g, '').split(',').map(v => v.trim()).filter(v => v);
        if (arcVars.length === 0 || arcVars.every(v => columns.includes(v))) return;
