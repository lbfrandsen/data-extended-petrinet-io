...
export function showRulesDialog({ title = '', message = '', onClose = null }) {
  ...
  checkbox.onchange = () => {
    pendingRules.integerGeneration = checkbox.checked
      ? 'defined'
      : (pendingRules.integerDistributionEnabled ? 'random_dist' : 'random_domain');
  ...
  intDistBtn.onclick = () => {
    ...
    intDistributionInputs.style.display = intDistEnabled ? 'flex' : 'none';
    intDistBtn.textContent = intDistEnabled ? 'Disable distribution' : 'Enable distribution';
    pendingRules.integerGeneration = intDistEnabled ? 'random_dist' : 'random_domain';
  };

  if (intDistEnabled) {
    pendingRules.integerGeneration = 'random_dist';
  } else if (pendingRules.integerGeneration !== 'defined') {
    pendingRules.integerGeneration = 'random_domain';
  }
  ...
  // Attach elements to modal and set spacing
  ...
  regexInput.style.marginBottom = '48px';
