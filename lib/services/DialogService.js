  ...
  return new Promise(resolve => {
    ...
    const renderConsumptionSection = (rowBinding) => {
      ...
      incomingArcs.forEach((connection, index) => {
        ...
        // Skip arcs with no variable (empty inscription) or whose variable is already bound by the query result row
        const rawInscription = connection?.businessObject?.arcInscription || '';
        const arcVar = rawInscription.replace(/[<>]/g, '').trim();
        if (!arcVar || columns.includes(arcVar)) return;
