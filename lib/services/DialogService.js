...
export function showDynamicMultiPrompt({
  title = 'Input',
  message = '',
  fields = [],
  validate,
  ...
  return new Promise(resolve => {
    ...
    const inputs = fields.map(field => {
      ...
      const updateOptions = (options) => {
        ...
        emptyOption.value = '';
        emptyOption.textContent = field.placeholder || 'Select binding';
      ...
      input.addEventListener('change', () => {
        if (typeof onChange === 'function') {
          const currentValues = {};
          inputs.forEach(({ key, input: inp }) => {
            currentValues[key] = inp.value;
          });
          const updatedOptions = onChange(currentValues) || {};
          inputs.forEach(({ key, updateOptions: upd }) => {
            if (Object.prototype.hasOwnProperty.call(updatedOptions, key)) {
              upd(updatedOptions[key]);
            }
          });
        }
      });
    ...
    // Initial onChange to set defaults
    if (typeof onChange === 'function') {
      const initialValues = {};
      inputs.forEach(({ key, input }) => {
        initialValues[key] = input.value;
      });
      const updatedOptions = onChange(initialValues) || {};
      inputs.forEach(({ key, updateOptions: upd }) => {
        if (Object.prototype.hasOwnProperty.call(updatedOptions, key)) {
          upd(updatedOptions[key]);
        }
...
export function showQueryRowAndConsumptionDialog({
  ...
  return new Promise(resolve => {
    ...
    const updateSaveBtnState = () => {
      if (!usesUserSelectedConsumption || !saveBtn) return;
      const allSelected = currentPrompts.length === 0 || currentPrompts.every(prompt => {
        const selectedValue = currentDropdownSelections[prompt.key];
        return selectedValue && selectedValue !== '';
      });

      saveBtn.disabled = selectedRowIndex === null || !allSelected;
      saveBtn.style.opacity = saveBtn.disabled ? '0.5' : '1';
    };

    const getFilteredPlans = () => {
      const selectedKeys = Object.entries(currentDropdownSelections).filter(([, value]) => value && value !== '');
      return selectedKeys.length === 0
        ? candidatePlans
        : candidatePlans.filter(plan => selectedKeys.every(([key, selectedValue]) => {
    ...
    const renderConsumptionSection = (rowBinding) => {
      if (rowBinding) {
        candidatePlans = getPlansForRow(rowBinding) || [];
      } else {
        candidatePlans = [];
      }
      ...
      if (!usesUserSelectedConsumption || !rowBinding) {
        updateSaveBtnState();
        return;
      }
      ...
      incomingArcs.forEach((connection, index) => {
        ...
        const currentValue = currentDropdownSelections[promptKey];
        ...
        select.addEventListener('change', () => {
          ...
          renderConsumptionSection(rowBinding);
        });
    ...
    saveBtn = createButton('Select Row', () => {
