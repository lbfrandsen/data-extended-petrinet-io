...
function drawGuardText(parentGfx, element, width, height) {
  ...
  if (bo.inputGuard && String(bo.inputGuard).trim()) {
    blocks.push({
      label: 'IN:',
      lines: formatGuardForDisplay(String(bo.inputGuard).trim())
    });
  }

  if (bo.outputGuard && String(bo.outputGuard).trim()) {
    blocks.push({
      label: 'OUT:',
      lines: formatGuardForDisplay(String(bo.outputGuard).trim())
    });
  }
  ...
  for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
    ...
    // Empty spacer line between IN / OUT blocks
