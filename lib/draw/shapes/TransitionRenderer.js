...
function drawGuardText(parentGfx, element, width, height) {
  ...
  const input = String(bo.inputGuard ?? '').trim();
  const output = String(bo.outputGuard ?? '').trim();

  const guardExpression = [input, output].filter(Boolean).join(' && ');

  if (!guardExpression) {
    return;
  }

  const lines = formatGuardForDisplay(guardExpression);
