...
export default class LabelRenderer {
  ...
  render(parentGfx, element) {
    ...
    if (element.type !== 'petri:empty_transition') {
      ...
      // Elias Storm Vedel Jørgensen - render arc inscription 
      if (element.type === 'petri:connection') {
        const ins = element.businessObject?.arcInscription ?? '';

        const insPos = this._calculateArcInscriptionPosition(element, element.businessObject || {});
        this._renderNameLabel(parentGfx, ins, insPos, attrs);
      }
    }
  }
  ...
  // Elias Storm Vedel Jørgensen - calculate position for arc inscription label
  _calculateArcInscriptionPosition(element, businessObject) {
    // reuse existing connection label position
    const base = this._calculateConnectionLabelPosition(element, businessObject);

    // push it down below the normal label
    return {
      ...base,
      y: base.y + 20
    };
