...
export function drawTransition(parentGfx, element, styles, simulationService, eventBus, sqlDialogService) {
  ...
  drawGuardText(parentGfx, element, width, height, eventBus, sqlDialogService);

  return rect;
}

// Renders the transition guard text.
function drawGuardText(parentGfx, element, width, height, eventBus, sqlDialogService) {
  ...
  text.addEventListener('dblclick', event => {
    ...
    promptAndSetGuard(element, eventBus, sqlDialogService);
  });
 ...
 * It does not try to parse nested brackets semantically.
 */
function formatGuardForDisplay(guard) {
  ...
  // If there was only one clause, close the bracket on same line
  if (parts.length === 1) {
    lines[0] = `${lines[0]} ]`;
  }
