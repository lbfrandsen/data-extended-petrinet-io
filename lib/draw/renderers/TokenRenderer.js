...
export default class TokenRenderer {
  ...
  render(parentGfx, element) {
    const markingCount = Array.isArray(element.businessObject?.marking)
      ? element.businessObject.marking.length
      : null;

    const tokens = Number.isFinite(markingCount)
      ? markingCount
      : (Number.isFinite(element.businessObject?.tokens) ? element.businessObject.tokens : 0);
  ...
  _renderTokenCount(parentGfx, cx, cy, count) {
    ...
    svgAttr(text, this.styles.computeStyle({}, {
      fill: 'black',
      fontSize: 12,
