import { parseType, validateValuesAgainstType, parsePlaceTypes } from '../../helpers/token-types.js';
...
export default class EditingProvider {
  ...
  update(element, newText, oldText, bounds) {
    if (element.type === "petri:transition") {
      ...
      const parsed = parseInscriptionLine(newText);
      ...
      for (const conn of siblings) {
        ...
        const otherTypes = parsePlaceTypes(otherPlace?.businessObject?.types || []);

        for (let i = 0; i < parsed.vars.length; i++) {
          for (let j = 0; j < vars.length; j++) {

            if (parsed.vars[i] === vars[j] && types[i] !== otherTypes[j]) {
              alert(`Variable "${parsed.vars[i]}" is already used with a different type on this transition.`);
              return;
            }
