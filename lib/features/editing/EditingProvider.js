...
import { parseValueByColor } from '../../helpers/valueColorUtils.js';
export default class EditingProvider {
  ...
  constructor(eventBus, directEditing, canvas, simulationService) {
    ...
    eventBus.on('element.dblclick', 1000, (event) => {
      ...
      const clickedGuardText = Boolean(
        originalTarget?.getAttribute?.('data-guard-text') === 'true' ||
        originalTarget?.closest?.('[data-guard-text="true"]')
      );

      // Guard text has its own dblclick behavior (open guard prompt).
      if (element.type === "petri:transition" && clickedGuardText) {
        return;
      }
  ...
  async addTokenPopup(place) {
    ...
    const values = rawParts.map((raw, i) => {
      return parseValueByColor(typeArr[i], raw);
    });
