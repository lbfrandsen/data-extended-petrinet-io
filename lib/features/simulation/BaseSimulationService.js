...
export default class BaseSimulationService {
    ...
    hasMasterBaselineChanges() {
        if (this.masterInitialTokenState.size === 0) {
            return false;
        }

        const currentDbSnapshot = this.databaseService?.snapshotDatabaseBytes?.() || null;
        const currentDbName = this.databaseService?.getDbName?.() || null;
        if (
            currentDbName !== this.masterInitialDbName ||
            !this._dbSnapshotsEqual(currentDbSnapshot, this.masterInitialDbSnapshot)
        ) {
            return true;
        }

        const places = this.elementRegistry.getAll().filter(el => el.type === "petri:place");
        if (places.length !== this.masterInitialTokenState.size) {
            return true;
        }
        ...
        return places.some(place => {
            ...
            return !this.deepEqual(
                getters.getPlaceMarking(place),
                this.masterInitialTokenState.get(place.id) || []
            );
        });
    }

    _dbSnapshotsEqual(left, right) {
        if (!left && !right) {
            return true;
        }
    ...
    removeTransitionRuntimeState(transitionOrId) {
        ...
        const hadFiredState = this.firedTransitions.delete(transitionId);
        const hadEnabledState = this.enabledTransitions.delete(transitionId);
        const hadDiagnostic = this.queryGuardDiagnostics.delete(transitionId);
        const originalLogLength = this.executionLog.length;

        this.executionLog = this.executionLog.filter(id => id !== transitionId);
        this.stepHistory = this.stepHistory.map(snapshot => ({
            ...snapshot,
            firedTransitions: Array.isArray(snapshot?.firedTransitions)
                ? snapshot.firedTransitions.filter(id => id !== transitionId)
                : []
        }));
        this.currentStepIndex = Math.min(this.currentStepIndex, this.executionLog.length);

        const changed = hadFiredState
            || hadEnabledState
            || hadDiagnostic
            || this.executionLog.length !== originalLogLength;
    ...
    setMasterBaselineToCurrentState() {
        this.saveMasterInitialTokenState();
    }

    rebaseInitialStateToCurrentMarking() {
        this.setMasterBaselineToCurrentState();
    }
    ...
    async fireTransition(element) {
        ...
        const actionResult = this.sqlParserService?.executeAttachedActionForTransition?.(element, plan?.binding || {});
    ...
    // check if a token matches place types, NOT just arity
    tokenMatchesPlaceType(tokenValues, placeType) {
        if (placeType.length === 0) {
            return tokenValues.length === 0; // no type ==> empty token
        }

        if (tokenValues.length !== placeType.length) {
            return false;
        }

        return tokenValues.every((value, index) => typedValueMatchesColor(placeType[index], value));
    }
    ...
    splitGuardClauses(guard) {
        ...
        for (let i = 0; i < String(guard ?? '').length; i++) {
            const char = guard[i];
    ...
    tryResolveOutputBinding(unresolved, index, currentBinding, guard) {
        ...
        for (const candidate of candidates) {
            if (!typedValueMatchesColor(color, candidate)) {
                continue;
            }
    ...
    canProduceOutgoingBindings(outgoingArcs, binding, guard) {
        ...
        for (const connection of (outgoingArcs || [])) {
            ...
            if ((arcInsRequirement.vars || []).length !== placeType.length) {
                return null;
            }
            ...
            for (let i = 0; i < placeType.length; i++) {
                ...
                if (varName && Object.prototype.hasOwnProperty.call(nextBinding, varName)) {
                    if (!typedValueMatchesColor(color, nextBinding[varName])) {
                        return null;
                    }
                    continue;
                }
                ...
                try {
                    ...
                    if (producedValue === undefined || !typedValueMatchesColor(color, producedValue)) {
                        return null;
                    }
