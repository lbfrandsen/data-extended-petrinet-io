import { showAlert, rules, showMultiPrompt } from '../../services/DialogService.js';
...
export default class BaseSimulationService {
    ...
    async fireTransition(element) {
        ...
        let plan = this.buildTransitionFiringPlan(element);
        if (!plan) {
            return;
        }
        ...
        if (rules.integerGeneration === 'defined') {
            plan = await this._askUser(element);
            if (!plan) return;
        }
    ...
    async _askUser(transition) {
        const candidatePlans = this.buildAllTransitionFiringPlans(transition);
        ...
        for (const connection of (candidatePlans[0].outgoingArc || [])) {
            ...
            const inscription = arcIns.text || `<${vars.join(',')}>`;
            const inscriptionLine = `${inscription}: ${placeType.join(',')}`;
            ...
            for (let i = 0; i < vars.length; i++) {
                ...
                const existingPrompt = prompts.find(prompt => prompt.varName === varName);
        ...
        if (prompts.length === 0) {
            return this.buildTransitionFiringPlan(transition);
        }

        const input = await showMultiPrompt({
            title: 'User input',
            message: `All arc inscriptions:\n${inscriptionLines.join('\n')}`,
            ...
            validate: values => {
                const parsedValues = this._parsePromptValues(values, prompts);
                ...
                if (!this.buildTransitionFiringPlanWithBinding(transition, parsedValues)) {
        ...
        const parsedValues = this._parsePromptValues(input, prompts);
        return this.buildTransitionFiringPlanWithBinding(transition, parsedValues);
    }

    _parseUserInputValue(value, color) {
        if (color === 'int') return Number(value);
        if (color === 'real') return Number(value);
        if (color === 'bool') return /^true$/i.test(String(value).trim());
        if (color === 'string') return String(value).trim().slice(1, -1);

        return value;
    }

    _parsePromptValues(values, prompts) {
        const parsedValues = {};

        prompts.forEach(prompt => {
            parsedValues[prompt.varName] = this._parseUserInputValue(values[prompt.varName], prompt.color);
        });
    ...
    buildTransitionFiringPlan(transition) {
        ...
        const queryBinding = this.buildQueryGuardBinding(transition);
    ...
    buildTransitionFiringPlanWithBinding(transition, initialBinding = {}) {
        ...
        return this.findBindingForTransition(
            ...
            0,
            currentMarkings,
            { ...queryBinding, ...initialBinding },
            [],
            this.getTransitionGuardExpression(transition),
            outgoingArcs
        );
    }

    buildAllTransitionFiringPlans(transition) {
        const incomingArcs = Array.isArray(transition.incoming) ? transition.incoming : [];
        const outgoingArcs = Array.isArray(transition.outgoing) ? transition.outgoing : [];
        const currentMarkings = new Map();
    ...
    buildQueryGuardBinding(transition) {
        ...
        this._setQueryGuardDiagnostic(transition, {
            status: 'ok',
            message: `Query ${queryId} returned ${queryCount} row(s).`
        });
        return {
            queryCount
        };
