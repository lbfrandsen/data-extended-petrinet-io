...
export default class BaseSimulationService {
    ...
    buildProducedToken(targetPlace, connection, binding) {
        ...
        const transition = this.getTransitionForConnection(connection);
    ...
    getPlaceType(place) {
        return parsePlaceTypeFromBusinessObject(place?.businessObject);
    }
    ...
    getTransitionForConnection(connection) {
        if (connection?.source?.type === 'petri:transition' || connection?.source?.type === 'petri:empty_transition') {
            return connection.source;
        }

        if (connection?.target?.type === 'petri:transition' || connection?.target?.type === 'petri:empty_transition') {
    ...
    randomValueForColor(color) {
        ...
        if (color === 'string') { // add string stuff
            return 's';
        }
    ...
    generateValueForVarWithGuard(varName, color, guard, binding) {
        ...
        if (color === 'bool') {
            ...
            return this.randomValueForColor(color); // safety
        }

        const bounds = this.extractNumericBounds(varName, guard, color);

        if (color === 'int') {
            const min = Math.max(Number.MIN_SAFE_INTEGER, bounds.min);
            const max = Math.min(Number.MAX_SAFE_INTEGER, bounds.max);
        ...
        const testBinding = { ...binding, [varName]: candidate };

        if (!this.evaluateGuardExpression(guard, testBinding)) {
            throw new Error(`Could not generate a value for "${varName}" that satisfies the guard.`);
        }
    }

    extractNumericBounds(varName, guard, color) {
        ...
        for (const clause of clauses) {
            ...
            const [, variable, op, literal] = match;

            if (variable !== varName) continue;

            const value = this.parseGuardLiteral(literal);
