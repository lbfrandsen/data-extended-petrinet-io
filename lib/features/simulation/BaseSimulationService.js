...
export default class BaseSimulationService {
    ...
    async fireTransition(element) {
        ...
        if (this.usesUserSelectedConsumption() || this.usesUserDefinedProduction()) {
            ...
            if (queryResult.rows !== null) {
                const fireable = this.getFireableRows(element, queryResult.rows);
                ...
                plan = this.buildTransitionFiringPlan(element);
            }
        }
    ...
    async _resolveUserDrivenPlan(transition) {
        ...
        const hasQuery = queryResult.rows !== null;

        if (hasQuery) {
            // Filter rows to those that are actually fireable given current tokens.
            const fireableRows = this.getFireableRows(transition, queryResult.rows);
            ...
            const result = await showQueryRowAndConsumptionDialog({
                ...
                getPlansForRow: (rowBinding) => this.buildAllTransitionFiringPlans(transition, rowBinding),
            ...
            const mergedBinding = { ...rowBinding, ...consumptionBinding };
        ...
        // No query attached — existing flow.
        const candidatePlans = this.usesUserSelectedConsumption()
            ? this.buildAllTransitionFiringPlans(transition, {})
            : null;
    ...
    buildTransitionFiringPlan(transition) {
        ...
        // No query attached — try with empty binding (existing behaviour).
        if (rows === null) {
            return this._buildFiringPlanWithRowBinding(transition, {});
        }

        // Query attached — try each row; return first fireable plan.
        for (const row of rows) {
            const plan = this._buildFiringPlanWithRowBinding(transition, this._buildRowBinding(row));
            if (plan) return plan;
        }
    ...
    // Returns { rows: null } when no query is attached (proceed normally),
    // { rows: [...] } when query ran (may be empty array = no results),
    ...
    getFireableRows(transition, rows) {
        const fireable = [];
        for (const row of rows) {
            const rowBinding = this._buildRowBinding(row);
