            ...
            for (let i = 0; i < vars.length; i++) {
                ...
                while (true) {
                    const input = await showPrompt({
                        ...
                        validate: (val) => {
                            const check = tokenValidationUtils.validateValuesAgainstType([color], [val]);
                            ...
                            else if (color === 'bool') parsed = /^true$/i.test(String(val).trim());
                            else if (color === 'string') parsed = String(val).trim().slice(1, -1);
                            else parsed = val;
                    ...
                    else if (color === 'bool') parsed = /^true$/i.test(String(input).trim());
                    else if (color === 'string') parsed = String(input).trim().slice(1, -1);
                    else parsed = input;
