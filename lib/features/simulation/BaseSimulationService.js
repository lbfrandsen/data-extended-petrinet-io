...
export default class BaseSimulationService {
    ...
    buildProducedToken(targetPlace, connection, binding) {
        console.log("Build produced token called");
    ...
    generateValueForVarWithGuard(varName, color, guard, binding) {
        console.log("Generate Value For Var guard called");
        ...
        if (color === 'int') {
            ...
            const user = false;
            console.log("Integer generation rule:", int_rule);
            ...
            if (int_rule === 'defined') {
                ...
                console.log('Integer generation rule:', int_rule);
                if (int_rule === 'random_domain') {
                    // 
                    const min = rules.integerDomainMin;
                    const max = rules.integerDomainMax;
                    ...
                    // normal distribution
                    const std = rules.integerStd;
                    const mean = rules.integerMean;
