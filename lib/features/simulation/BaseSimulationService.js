    ...
    generateValueForVarWithGuard(varName, color, guard, binding) {
        ...
        if (color === 'int') {
            ...
            if (normal_distribution) {
                ...
                const value = Math.round(mathService.randomNormal(mean, std));

                if (value < min) return min;
                if (value > max) return max;
                return value;
            }
            return mathService.randomIntBetween(min, max);
        }
    }
        ...
        if (color === 'real') {
            ...
            if (real_distribution) {
                ...
                const value = Number(mathService.randomNormal(mean, std).toFixed(decimals));
