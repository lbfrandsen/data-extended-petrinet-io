export default class KeyboardShortcuts {
  ...
  _handleKeyDown(event) {
    // Do not intercept browser/editor text shortcuts while user is typing.
    if (this._isTextInputActive(event)) {
      return;
    }
    ...
    const isCmdKey = event.ctrlKey || event.metaKey;

    // Undo model command: Cmd/Ctrl+Z
    if (isCmdKey && !event.shiftKey && key === 'z') {
      this._handleUndo();
      event.preventDefault();
      return;
    }

    // Redo model command: Ctrl+Y (Win/Linux) or Cmd+Shift+Z (macOS)
  ...
  _handleUndo() {
    // Avoid no-op undo calls when history is empty.
