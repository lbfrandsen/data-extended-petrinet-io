...
export function buildTransitionXml(element, { includeQueryBindings = true } = {}) {
    ...
    const queryGuardId = includeQueryBindings
        ? String(element.businessObject?.queryGuardId ?? '').trim()
        : '';
    const actionId = String(element.businessObject?.actionId ?? '').trim();
