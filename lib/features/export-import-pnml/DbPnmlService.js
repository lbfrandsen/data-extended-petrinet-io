...
export default class DbPnmlService {
  ...
  exportCombined(filename = 'petri-net.dbpnml') {
  ...
  async importCombinedFile(file) {
    ...
    try {
      ...
      throw new Error('The selected .dbpnml file is not valid JSON.');
    }

    if (parsed?.format !== 'dbpnml' || typeof parsed?.pnml !== 'string' || !parsed?.db?.data) {
      throw new Error('The selected .dbpnml file is missing PNML or database data.');
    }
    ...
    if (!loaded) {
      throw new Error('The database payload inside the .dbpnml file could not be loaded.');
    }
  }
