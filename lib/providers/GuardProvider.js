...
function parseClause(expression, variableMeta) {
  ...
  const leftLenMatch = left.match(/^(?:len|length)\((.*?)\)$/);
  const rightLenMatch = right.match(/^(?:len|length)\((.*?)\)$/);

  if (leftLenMatch || rightLenMatch) {
    const varName = leftLenMatch ? leftLenMatch[1].trim() : rightLenMatch[1].trim();
    const varMeta = variableMeta.get(varName);

    if (!varMeta) {
      throw new Error(`Unknown variable "${varName}" in length expression.`);
    }

    if (varMeta.type !== 'string') {
      throw new Error(`len(...) is only supported for string variables.`);
    }

    const expr = leftLenMatch ? right : left;

    if (!/^[A-Za-z0-9_\s+\-*/%().]+$/.test(expr)) {
      throw new Error('String length must be an integer or numeric expression.');
    }
    ...
    for (const name of identifiers) {
      ...
      if (!meta) {
        throw new Error(`Unknown variable "${name}" in length expression.`);
      }

      if (meta.type !== 'int' && meta.type !== 'real') {
        throw new Error(`Variable "${name}" in length expression must be int or real.`);
      }
    }

    if (!['=', '==', '!=', '>', '<', '>=', '<='].includes(operator)) {
      throw new Error(`Unsupported operator "${operator}" for string length.`);
    }
