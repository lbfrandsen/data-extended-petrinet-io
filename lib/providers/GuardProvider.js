...
import { getQueryById } from '../services/SqlDialogService.js';
...
function buildQueryVariableTypeMap(transition) {
  ...
  const queryText = getQueryById(queryId);
  const isCountQuery = queryText.trim().toLowerCase().startsWith('select count(') || queryText.trim().toLowerCase().startsWith('select count (');
