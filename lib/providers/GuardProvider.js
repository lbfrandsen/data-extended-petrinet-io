...
function promptAndSetGuard(transition, eventBus, direction) {
  ...
  if (typedConnections.length === 0) {
    window.alert(`No typed ${direction} arcs found yet. Connect one typed place first.`);
    return;
  }
  ...
  const input = window.prompt(
    buildPromptMessage(direction, typedConnections, variableTypes),
    current
  );
...
export function promptAndSetInputGuard(transition, eventBus) {
  promptAndSetGuard(transition, eventBus, 'input');
}

export function promptAndSetOutputGuard(transition, eventBus) {
