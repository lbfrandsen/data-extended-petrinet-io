...
export default class PlaceTypingProvider {
    ...
    async promptAndSetPlaceType(place) {
        ...
        const prev = place.businessObject.placeType;

        place.businessObject.placeType = selected;
        if (prev != selected) {
            place.businessObject.marking = [];
            console.log("Tokens: ", place.businessObject.marking);
            ...
            for (const arc of incomingArcs) {
                ...
                if (transition?.businessObject) {
                    transition.businessObject.guardExpression = '';
