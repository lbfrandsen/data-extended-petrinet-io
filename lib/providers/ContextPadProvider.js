import { promptAndSetPlaceType } from '../providers/PlaceTypingProvider';
...
export default class CustomContextPadProvider {
  ...
  getContextPadEntries(element) {
    ...
    const createAdjacent = (type) => {
      ...
      if (type === 'petri:place') {
        shapeConfig.businessObject = { tokens: 0, place_type: null, marking: [] };
      }
    ...
    if (element.type === 'petri:place') {
      entries.transition = {
        ...
        entries.setType = {
          ...
          action: {
            click: () => {
              promptAndSetPlaceType(element, this.eventBus);
            }
          }
        };
