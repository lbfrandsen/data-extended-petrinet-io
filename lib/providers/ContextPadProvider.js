import { promptAndSetPlaceTypes } from '../providers/PlaceTypingProvider';
import { promptAndSetArcInscription, registerArcInscriptionSync } from '../providers/ArcInscriptionProvider.js';
import { promptAndSetInputGuard, promptAndSetOutputGuard } from './GuardProvider.js';
export default class CustomContextPadProvider {
  ...
  constructor(eventBus, connect, contextPad, modeling, elementFactory, create, popupMenu, idCounterService, selection, simulationService, directEditing) {
  ...
  getContextPadEntries(element) {
    ...
    const createAdjacent = (type) => {
      ...
      if (type === 'petri:place') {
        shapeConfig.businessObject = { tokens: 0, types: [], marking: [] };
      }
      ...
      promptAndSetArcInscription(connection, this.eventBus);
    ...
    if (element.type === 'petri:place') {
      entries.transition = {
        ...
        entries.setType = {
          ...
          action: {
            click: () => {
              promptAndSetPlaceTypes(element, this.eventBus);
            }
          }
        };
      ...
      entries.setInputGuard = {
      group: 'tools',
      className: 'bpmn-icon-gateway-none',
      title: 'Set input guard',
      action: {
        click: () => {
          promptAndSetInputGuard(element, this.eventBus);
        }
      }
    },
    entries.setOutputGuard = {
      group: 'tools',
      className: 'bpmn-icon-gateway-xor',
      title: 'Set output guard',
      action: {
        click: () => {
          promptAndSetOutputGuard(element, this.eventBus);
        }
      }
    };
    }

    // // Elias Storm Vedel Jørgensen - added entry for setting arc inscriptions
    // if (element.type === 'petri:connection') {
    //   entries.setInscription = {
    //     group: 'tools',
    //     className: 'bpmn-icon-end-event-error',
    //     title: 'Set arc inscription',
    //     action: {
    //       click: () => {
    //         promptAndSetArcInscription(element, this.eventBus);
    //       }
    //     }
    //   };
    // }
