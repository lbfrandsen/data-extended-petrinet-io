...
export function parseInscriptionLine(line) {
    ...
    if (!match) {
        throw new Error('Invalid inscription. Use <x>, <x,y>, <x>2 or <x>^2.');
    }
    ...
    for (const v of vars) {
        if (!/^[A-Za-z][A-Za-z0-9]*$/.test(v)) {
            throw new Error(`Invalid variable name "${v}". Use letters and numbers like x, y, x3.`);
        }
    }
...
function buildArcInscriptionFromPlace(connection, place) {
    const types = parsePlaceTypes(place?.businessObject?.types || []); // get place types
    const vars = createDefaultArcVars(connection, types.length); // create variable names based on number of types and offset
