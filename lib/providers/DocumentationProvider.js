...
<ul>
  <li>Ricardo Marin Perez</li>
  <li>Andrea Burattin (Supervising Professor)</li>
</ul>
...
<ul>
  ...
  <li>If query evaluation fails (no database, empty query, removed query, SQL error), the transition is disabled.</li>
...
<ul>
  ...
  <li><strong>Reset</strong> (<code>r</code>): opens a dialog to choose between reset modes:
    <ul>
      <li><strong>Soft Reset</strong>: restores token marking to the start of the current simulation run.</li>
      <li><strong>Master Reset</strong>: restores token marking to the saved master baseline.</li>
      <li><strong>Set New Master Baseline</strong>: when the current marking differs from the master baseline, saves the current marking and database snapshot as the new master baseline without resetting immediately.</li>
    </ul>
  </li>
  <li>Soft Reset and Master Reset clear simulation history, fired-transition markers, and exit simulation mode.</li>
</ul>
...
<ul>
  ...
  <li>After import, toggle simulation on to start a fresh session from the imported model state, with the saved master-reset baseline intact.</li>
</ul>
...
<ul>
  ...
  <li><code>r</code>: Open reset dialog (Soft Reset, Master Reset, or Set New Master Baseline when applicable). Requires simulation to be active.</li>
