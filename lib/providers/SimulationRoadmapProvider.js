export default class SimulationRoadmapProvider {
  static $inject = ['eventBus', 'simulationService'];

  constructor(eventBus, simulationService) {
  ...
  render() {
    ...
    summary.textContent = `Status: ${snapshot.active ? 'active' : 'inactive'} | Enabled: ${snapshot.enabledTransitionCount} | Depth: ${snapshot.stepHistoryDepth}`;
    ...
    const currentStep = snapshot.executionLog.length;
    ...
    if (!snapshot.executionLog.length) {
      ...
      rows.forEach(({ step, text }) => {
        ...
        } else if (!canJump || step > currentStep) {
          rowButton.disabled = true;
        } else {
