...
import { formatTransitionLabel } from './LegendProvider.js';
...
export default class SimulationRoadmapProvider {
  ...
  _findTransitionById(transitionId) {
    const normalizedId = String(transitionId ?? '').trim();

    if (!normalizedId || typeof this.elementRegistry?.getAll !== 'function') {
      return null;
    }
    ...
    return this.elementRegistry
      ...
      .find((element) =>
        ...
        String(element?.id ?? '').trim() === normalizedId
      ) || null;
  }

  _formatExecutionLogTransition(transitionId) {
    const transition = this._findTransitionById(transitionId);
    return formatTransitionLabel(transition || { id: transitionId });
  }
  ...
  render() {
    ...
    title.textContent = 'EXECUTION ROADMAP';
    ...
    hint.textContent = canJump
      ? 'Click an execution step to jump directly.'
      : 'Toggle simulation mode to jump between steps.';
    ...
    if (!snapshot.executionLog.length) {
      ...
      empty.textContent = 'No execution steps yet.';
      ...
      const rows = [
        ...
        ...snapshot.executionLog.map((transitionId, index) => ({
          step: index + 1,
          text: `Step ${index + 1} - Fired ${this._formatExecutionLogTransition(transitionId)}`
        }))
      ];
