...
export default class PlaceTokenHoverProvider {
    ...
    buildOverlayContent(place) {
        ...
        // TODO: Implement tokens with unique ID's. Currently on hover, they're just printed as text with their values, not identifiable as separate tokens
        const lines = ['Tokens:'];
        grouped.forEach(entry => {
            lines.push(entry.count > 1 ? `${entry.count}: ${entry.text}` : `${entry.text}`);
        });
