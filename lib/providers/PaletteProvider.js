import { showAlert, showConfirmDialog, showPrompt, showResetModeDialog } from '../services/DialogService.js';
...
export default class CustomPaletteProvider {
  ...
  async _resetSimulation() {
    ...
    while (this.simulationService.isSimulationActive()) {
      const resetMode = await showResetModeDialog({
        title: 'Reset Simulation',
        message: 'Choose reset behavior:\n- Master Reset: restore marking from the saved master baseline.\n- Soft Reset: restore marking from before the current simulation run.',
        canSetMasterBaseline: this.simulationService.hasMasterBaselineChanges?.() ?? false
      });
      ...
      if (resetMode === 'set-master-baseline') {
        const confirmed = await showConfirmDialog({
          title: 'Set New Master Baseline',
          message: 'This will overwrite the current master reset baseline with the current token marking and database snapshot. The previous master baseline cannot be restored from this action.\n\nNo reset will happen immediately. After confirming, you will return to the reset dialog.',
          confirmLabel: 'Yes',
          cancelLabel: 'No',
          confirmKind: 'danger'
        });
        ...
        if (confirmed) {
          this.simulationService.setMasterBaselineToCurrentState();
        }
        continue;
      }
