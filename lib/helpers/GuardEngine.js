...
export function evaluateGuardAstValue(ast, binding = {}, options = {}) {
...
// Evaluates an AST against bindings.
// Returns a strict boolean (or `undefined` only in partial mode when undecidable).
export function evaluateGuardAst(ast, binding = {}, options = {}) {
  const partial = Boolean(options.partial);
  const value = evaluateGuardAstValue(ast, binding, options);
