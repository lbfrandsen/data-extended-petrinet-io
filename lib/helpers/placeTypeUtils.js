...
// Parse raw place type metadata.
export function parsePlaceType(placeType) {
    if (Array.isArray(placeType)) {
        throw new Error('Legacy place type arrays are not supported. Use placeType as a single string value.');
    }
...
export function getRawPlaceType(businessObject) {
    ...
    return businessObject.placeType ?? businessObject.place_type ?? null;
}
